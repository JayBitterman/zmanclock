...
const hours = ["נץ", "א", "ב", "ג", "ד", "ה", "ו", "ז", "ח", "ט", "י",
  ...
  "ט", "י", "יא",// Import necessary modules
import * as Astronomy from 'https://cdn.jsdelivr.net/npm/astronomy-engine@2.1.19/+esm';
import * as hcal from 'https://cdn.jsdelivr.net/npm/@hebcal/core@5.5.0/+esm';
import * as jDate from 'https://cdn.jsdelivr.net/npm/jewish-date@2.0.12/+esm';
import { point, booleanPointInPolygon } from 'https://cdn.jsdelivr.net/npm/@turf/turf@7.1.0/+esm';
...
function positionMoon() {
  ...
  const cos_gamma = Math.sin(delta_s)*Math.sin(delta_m)
                  + Math.cos(delta_s)*Math.cos(delta_m)*Math.cos(alpha_s - alpha_m);
  const gamma = Math.acos(cos_gamma);

  const sinP = (Math.cos(delta_s) * Math.sin(alpha_s - alpha_m)) / Math.sin(gamma);
  const cosP = (Math.sin(delta_s) * Math.cos(delta_m)
              - Math.cos(delta_s) * Math.sin(delta_m)*Math.cos(alpha_s - alpha_m))
             / Math.sin(gamma);
  const P = Math.atan2(sinP, cosP);

  // 6. Compute the Moon’s orientation so it matches the real sky:
  //    • Northern hemisphere: orientation = P - q
  //    • Southern hemisphere: orientation = P + q
  const orientation = usr_lat >= 0
    ? P - q
    : P + q;
  ...
  // Rotate the moon (the +90° offset keeps the terminator vertical at zero)
  ...
  if (nextMoonset.date < nextMoonrise.date) {
    progressAngle = 180 * (date - prevMoonrise.date) 
                  / (nextMoonset.date - prevMoonrise.date);
  } else {
    progressAngle = 180 + 180 * (date - prevMoonset.date) 
                     / (nextMoonrise.date - prevMoonset.date);
  }
  progressAngle %= 360;
...
function setBackgroundColor(){
  function calcGradient(base){
    ...
    return`radial-gradient(circle at ${set_rise}% 49vh, ${redGrad} 0%, ${orangeGrad} 5%, ${tan} 10%, ${blueTan} 20%, ${darkPurple} 30%, ${base} 100%)`;
  }
...
function init() {
  ...
  window.addEventListener('calendarDateSelected', e=>{
    const picked = e.detail.gDate;          // a JS Date at 00:00 local
    const today  = new Date(); today.setHours(0,0,0,0);
  
    // reuse the clock’s own “days_ahead” mechanic
    days_ahead = Math.round((picked - today)/86400000);   // 1 day = 86 400 000 ms
    updateResetButton();                                  // keeps the ⟳ button logic
    updateClock();
  });
}
